    ...
    while ((i = getopt(argc, argv, "d:f:i:clmrsSwh")) != EOF){
        switch(i){
            ...
            case 'f':
                ...
                break;
            case 'i':
                pCmd->interval = long(atof(optarg)*SEC_PER_US);
                if(pCmd->interval<=0)
                    pCmd->interval = SEC_PER_US;
                break;
            case 'l':
...
bool clear(const char* path, FileType file_type){
    int  fd = 0;
    DIR  *dir = NULL;
    char file_name[256];
    void *pbase = NULL;
...
bool normalStat(const char* path, FileType file_type, CacheStat& stat_cache){
    int fd = 0;
    int pageIndex;
    int pageCount;
    int inCache = 0;
...
int realStat(const char *path, long interval, FileType file_type, bool isSuppress = false){
    CacheStat stat;
    int pre = 0;
    bool first = true;
    bool ret = false;
...
bool lock(const char* path, FileType file_type){
    int fd = 0;
    DIR *dir = NULL;
    void *pbase = NULL;
    struct dirent *dp = NULL;
...
bool warmup(const char *path, FileType file_type){
    int fd = 0;
    DIR *dir = NULL;
    int time_used = 0;
    char file_name[256];
